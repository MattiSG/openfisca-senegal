...
These elements are described in different folders. All the modelling happens within the `openfisca_senegal` folder.
...
```sh
git clone git://github.com/MattiSG/openfisca-senegal.git.git
cd openfisca-senegal.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_senegal
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_senegal/situation_examples/couple.json \
